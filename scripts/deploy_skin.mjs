 ...
 * Installs by copying pet-skins/<petId>/ into ~/.ai-pet/pets/<petId>/.
 */
import {
  access,
  cp,
...
const SKINS_ROOT = path.resolve(__dirname, '..', 'pet-skins');
 ...
 * @param {string} packageRoot - Absolute path to `pet-skins/`.
...
const loadPets = async () => {
  const deployable = await listDeployablePets(SKINS_ROOT);
  return Promise.all(
...
const main = async () => {
  ...
  if (pets.length === 0) {
    console.error(`no pet packages found under ${SKINS_ROOT}`);
    process.exit(1);
  }
