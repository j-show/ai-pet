...
/**
 * Open an aipet:// URL.
 * - During `pnpm dev`, forwards to the Vite dev bridge on :1420
...
try {
  ...
  try {
    ...
    console.error('');
    console.error('Tips:');
    console.error('  1. Run `pnpm dev`, then retry this command');
    console.error(
      '  2. Or run `pnpm build:app`, open AI Pet.app once, then retry'
    );
    process.exit(1);
  }
}
